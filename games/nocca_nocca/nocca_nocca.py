...
class NoccaNocca(AlternatingGame):
    def __init__(self, initial_player=None, max_steps=None, seed=None, render_mode='human', alfa_D=10, alfa_B=5, dis_type = 'Vertical'):
        super().__init__()
        ...
        #Valores de para la funcion de evaluacion
        self.alfa_D = alfa_D
        self.alfa_B = alfa_B
    ...
    def eval(self, agent):
        ...
        # Convertir nombre del agente a ID de jugador
        player = self.agent_name_mapping[agent]
        
        # Encontrar el oponente
        opponent_player = self.board._opponent(player)
        
        # Calcular métricas para la evaluación
        my_dist = self.board.dist_to_opponent_home(player, dis_type=self.dis_type)
        opp_dist = self.board.dist_to_opponent_home(opponent_player, dis_type=self.dis_type)
        my_blocked = self.board.count_blocked_pieces(opponent_player) #porque el agente cuenta las piezas bloqueadas del oponente
        opp_blocked = self.board.count_blocked_pieces(player)

        # Aplico la fórmula de evaluación
        # E(s) = -α_D * D(s) + α_B * B(s)
        # donde D(s) = (distancia_mía - distancia_oponente)
        # y B(s) = (bloqueadas_por_mi - bloqueadas_por_oponente)
        
        distance_diff = my_dist - opp_dist  # D(s)
        blocked_diff =  my_blocked - opp_blocked  # B(s)
        
        if my_blocked or opp_blocked == 5:
            self.alfa_D=0
        else:
            self.alfa_D = self.alfa_D    

        if my_dist or opp_dist == 0:
            self.alfa_B=0
        else:
            self.alfa_B = self.alfa_B        

        evaluation = -self.alfa_D * distance_diff + self.alfa_B * blocked_diff
